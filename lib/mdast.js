...
function mdastToMarkdownAnsiOptions (format) {
  ...
  return {
    ...
    bullet: '-',
    handlers: /** @satisfies {Partial<Record<import('mdast').Nodes["type"], import('mdast-util-to-markdown').Handle>>} */ ({
      text (node) {
        return node.value;
      },
      code (node) {
      ...
      list (node, _parent, state, info) {
        const items = node.children.map((/** @type {import('mdast').ListItem} */ child, /** @type {number} */ i) => {
