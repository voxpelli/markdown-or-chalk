...
const outputStyles = {
  ...
  get 'ansi-rich' () {
    try {
      ...
      // Narrowed to the two peers, matched as the *unresolved specifier* rather
      // than anywhere in the message. Any resolution failure in this style's
      // subtree lands here — a half-installed mdast-util, a typo in an internal
      // import — and blaming those on a missing optional peer would send someone
      // off installing packages they already have. A substring test is not
      // enough for that, because the ESM message also names the importing file:
      // "Cannot find package 'lowlight' imported from .../node_modules/emphasize/
      // lib/index.js" contains 'emphasize' while the missing package is not it.
      // Both codes quote the specifier, so read it out.
      const message = /** @type {Error} */ (cause)?.message ?? '';
      const [, specifier] = /Cannot find (?:package|module) '([^']+)'/.exec(message) ?? [];

      if (specifier !== 'boxen' && specifier !== 'emphasize') throw cause;
