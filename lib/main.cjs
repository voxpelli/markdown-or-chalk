...
const outputStyles = {
  ...
  get 'ansi-rich' () {
    try {
      ...
      // Narrowed to the two peers by name. Any resolution failure anywhere in
      // this style's subtree lands here — a half-installed mdast-util, a typo in
      // an internal import — and blaming those on a missing optional peer would
      // send someone off installing packages they already have. Both codes name
      // the specifier they could not resolve.
      const message = /** @type {Error} */ (cause)?.message ?? '';

      if (!message.includes('boxen') && !message.includes('emphasize')) throw cause;
