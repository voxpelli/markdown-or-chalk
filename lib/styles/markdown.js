...
const markdownOutput = /** @satisfies {MarkdownStyledOutput} */ (/** @type {const} */({
  ...
  hyperlink (text, url) {
    ...
    // `text` is already-formatted output here, exactly as it is for bold() and
    // the rest — `hyperlink(bold(x), url)` has to keep working — so only the
    // two characters that would break the link *structure* are escaped, not
    // markdown syntax in general. The url is escaped the way the serializer
    // does it: angle brackets when it contains spaces, backslashes otherwise.
    const escapedText = text.replaceAll(/[[\]]/g, String.raw`\$&`);
    const escapedUrl = /\s/.test(safeUrl)
  ...
  list (items) {
    ...
    // `this?.indent` so an overridden indent() is honoured, falling back when
    // the method is detached from its styler
