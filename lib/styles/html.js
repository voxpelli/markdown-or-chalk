 ...
 * @returns {string}
 */
function renderCell (cell, cellTag, state, info) {
  return `<${cellTag}>${state.containerPhrasing(cell, { ...info, before: '', after: '' })}</${cellTag}>`;
}
 ...
 * @returns {string}
 */
function renderRow (row, cellTag, state, info) {
  const cells = (row?.children ?? []).map(cell => renderCell(cell, cellTag, state, info));
...
const htmlOptions = createOptionsCache(format => ({
  handlers: /** @satisfies {{ [K in Nodes['type']]?: Handle<Extract<Nodes, { type: K }>> }} */ ({
    ...
    inlineCode (node) {
      // Through `format` like the other styling nodes — `code()` already
      // escapes, so this must not escape first or the markup arrives doubled
      return format.code(node.value);
    },
    code (node) {
      // Restricted rather than merely escaped: this lands in a class name, and
      // `lang` is free-form on a programmatically built tree
      const languageClass = node.lang && /^[\w.+-]+$/.test(node.lang)
    ...
    listItem (node, _parent, state, info) {
      // Reached only when a listItem is rendered outside a list — `list` below
      // handles its own children. Without this the default markdown handler
      // takes it and prints a `* ` bullet into the page
      return `<li>${state.containerFlow(node, info)}</li>`;
    },
    definition () {
    ...
    table (node, _parent, state, info) {
      ...
      const head = headerRow ? `<thead>\n${renderRow(headerRow, 'th', state, info)}\n</thead>\n` : '';
      const body = bodyRows.length
        ? `<tbody>\n${bodyRows.map(row => renderRow(row, 'td', state, info)).join('\n')}\n</tbody>\n`
        : '';

      return `<table>\n${head}${body}</table>`;
    },
    // `table` renders its own rows and cells, so these are reached only when one
    // is the node being serialized. The terminal styles get them free from
    // `gfmTableToMarkdown`; without them a tree that renders everywhere else
    // throws here alone
    tableRow (node, _parent, state, info) {
      return renderRow(node, 'td', state, info);
    },
    tableCell (node, _parent, state, info) {
      return renderCell(node, 'td', state, info);
    },
  }),
}));
