...
/**
 * The lean terminal renderers — chalk only. Syntax highlighting and boxed code
 * blocks live in the `ansi-rich` style, which is where boxen and cli-highlight
...
export function ansiRenderers (format) {
  return {
    ...
    codeBlock: value => '\n' + indentText(value).split('\n').map(line => chalk.dim('│') + line).join('\n') + '\n',
    inlineCode: value => chalk.cyan(value),
